...
- Server-side validation for contact fields, email format, payload length, and consent acceptance.
- Database fields for consent acknowledgement and region metadata on contact submissions.
...
The current implementation is a Vite single-page application suitable for a Vercel preview build. The home page does not require new client dependencies. The contact endpoint uses the existing Express API and Postgres migration flow.
