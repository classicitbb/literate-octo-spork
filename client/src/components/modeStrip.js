...
export function initModeStrip(role, onSwitch) {
  ...
  strip.innerHTML = `
    <button class="mode-btn active" id="modePatient" data-mode="patient">🏥 Patient</button>
    ${canCSR ? `<button class="mode-btn" id="modeCSR" data-mode="csr">📋 CSR</button>` : ''}
    ${canAdmin ? `<button class="mode-btn" id="modeAdmin" data-mode="admin">📊 Admin</button>` : ''}
    ${role === 'dev' ? `<button class="mode-btn" id="modeDev" data-mode="dev">🛠 Dev</button>` : ''}
  `;
